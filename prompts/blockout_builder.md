...
- give the contract named source-image evidence; a room-space `frame` with `origin_xyz`, orthonormal `x_axis_xy` and `y_axis_xy`, and positive `size_xyz`; an ordered room-space `footprint_xy`; unit `front_xy` equal to the frame y axis; named regions with room-space bounding boxes; numeric relationships; and `ownership.children` set to `external` or `included`;
